  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 pb-10">
      {data.map((item) => (
        <Card
          key={item.id}
          className="bg-primary/10 rounded-xl cursor-pointer hover:opacity-75 transition border-0 overflow-hidden"
        >
          <Link href={`/chat/${item.id}`} className="block h-full">
            <CardHeader className="flex flex-col items-center justify-center text-center space-y-2 p-4">
              ...
              <div>
                <p className="font-bold text-muted-foreground  text-sm line-clamp-1">
                  {item.name}
                </p>
